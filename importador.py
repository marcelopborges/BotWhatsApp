...
class IMPORTADOR:
    ...
    def import_receita_prev():

        ####DADOS KM_PREV####
        data_receita_prev = pd.read_csv(f'{IMPORTADOR.path}PrevReceita.csv', sep=';')
        base_receita_prev = pd.DataFrame(data_receita_prev)
        base_receita_prev.to_sql('RECEITA_PREV',IMPORTADOR.conn, if_exists='replace', index=False)